...
ipcRenderer.on('pending-tasks', (event, args) => {
    ...
    //Passo 4: renderizar as tarefas pendentes do documento index.html
    /*
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//CRUD Read - alterar os dados do banco
/*
Passo1
  1.1 - Criar o botão Editar na lista de tarefas
  1.2 - Criar a função editarTarefa e testar no console
  1.3 - acrescentar a diretiva 'unsafe-inline'
  1.4 - testar a passagem do id como parâmetro
  1.5 - Criar duas variáveis de apoio para reutilização do formulário nos métados adicionar e editar uma tarefa
...
function editarTarefa(id) {
    ...
    //passos 5 e 6 - receber a confimação do update e renderizar novamente
...
function excluirTarefa(id) {
    ...
    //Passo 2 - confirmar a exclusão(main) -> enviar este ao main junto com o id da tarefa a ser excluida
    ipcRenderer.send('delete-task', id)
}

//PASSO 4 -> Receber a confirmação de exclusão e renderizar novamente a lista de tarefas pendentes
ipcRenderer.on('delete-task-success', (even, args) =>{
    console.log(args)//teste de recebimento
    //atualizar a lista de tarefas pendentes usando um filtro no array para remoer a tarefa excluida
