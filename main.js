...
ipcMain.on('new-task', async (event, args) => {
  ...
  //validação de campo obrigatorios
  if (args.nome === "") {
    //fins didáticos - desnecessario
    ...
    //usar o modal(dialog) do sistema operacional para enviar uma mensagem ao usuário comfirmando que a tarefa foi salva
    /*
  //fins didáticos - desnecessario
  dialog.showMessageBox(win, {
  type: 'info',
...
ipcMain.on('update-task', async (event, args) => {
  ...
  if (args.nome === "") {
    ...
    //enviar a confirmação para renderer junto com a tarefa editada (passo 5 do slide)
...
//CRUD Delete >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//Passo 2(slide) - receber o pedido do renderer para excluir uma tarefa no banco de dados
ipcMain.on('delete-task', async (event, args) => {
  ...
  if (response === 1) {
    const tarefaExcluida = await Tarefa.findByIdAndDelete(args) //Passo 3 excluir a tarefa do banco e enviar a lista de tarefas pendentes
